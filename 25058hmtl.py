...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .shell { max-width: 980px; margin: 0 auto; padding: 24px; display: grid; grid-template-rows: auto 1fr auto; gap: 16px; height: 100dvh; }
...
<body>
  <div class="shell">
    ...
    <div id="chatbox">
      ...
      <div class="toolbar" style="padding:8px 8px 0;">
        ...
        <button type="button" class="chip" data-q="Supermercados">Supermercados</button>
        <button type="button" class="chip" data-q="Gimnasios">Gimnasios</button>
        <button type="button" class="chip" data-q="Spas">Spas</button>
  ...
  <script>
    ...
    // Persistencia de toggles/hora
    const prefs = JSON.parse(localStorage.getItem("uiPrefs") || "{}");
    ["plan-hoy","use-geo","prefer-open","wheelchair","reservas"].forEach(id=>{
    ...
    $("#clear").addEventListener("click", () => {
      ...
      chatlog.innerHTML = "";
      const msg = "Conversación reiniciada. ¿Qué te gustaría buscar?";
      addBot(msg);
    ...
    const CANON_MAP = [
      ...
      { re: /\brestaurantes?\b/i, canon: "Restaurantes" },
      { re: /\b(parques?\s+de\s+atracciones?)\b/i, canon: "Parques de atracciones" },
      { re: /\bcentros?\s+comerciales?\b/i, canon: "Centros comerciales" },
      { re: /\btiendas?\b/i, canon: "Tiendas" },
      { re: /\bsupermercados?\b/i, canon: "Supermercados" },
      { re: /\bgimnasios?\b/i, canon: "Gimnasios" },
    ...
    $("#use-geo").addEventListener("change", (e) => {
      if (e.target.checked) requestGeo();
    ...
    $("#composer").addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (data && data.response) {
          ...
          saveHistory();
          enhanceNoMatch(raw, data.response, lastBubble);
        } else {
    ...
    function isNoMatch(text) {
      ...
      return t.includes("no encontr") || t.includes("no tengo datos");
    }
    function enhanceNoMatch(userRaw, assistantText, bubbleEl) {
      ...
      if (suggestions.length) {
        const sug = suggestions.map(s => `• ${s}`).join("<br>");
        const html = `<br><div class="meta" role="note">¿Te refieres a alguna de estas búsquedas?<br>${sug}</div>`;
